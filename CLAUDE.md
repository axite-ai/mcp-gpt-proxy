...
The MCP GPT Proxy transparently proxies OAuth 2.0 authentication flows from ChatGPT to upstream MCP servers, implementing the [MCP Authorization specification](https://modelcontextprotocol.io/specification/draft/basic/authorization).

### OAuth Endpoints

The following OAuth endpoints are automatically proxied:

**Discovery Endpoints** (root level, per MCP spec):
- `/.well-known/oauth-protected-resource` - Protected Resource Metadata (RFC 9728)
- `/.well-known/oauth-authorization-server` - Authorization Server Metadata (RFC 8414)
- `/.well-known/openid-configuration` - OpenID Connect Discovery (optional)
...
Discovery documents fetched from upstream are automatically rewritten to replace upstream URLs with proxy URLs. This ensures OAuth flows complete correctly through the proxy layer.
...
```json
{
  "resource": "https://upstream.com/mcp",
  "authorization_servers": ["https://upstream.com"]
}
```
...
```json
{
  "resource": "https://proxy.com/mcp",
  "authorization_servers": ["https://proxy.com"]
}
```

### HEAD Request Compatibility

The proxy automatically converts HEAD requests to GET requests when forwarding to upstream, as some MCP servers don't support HEAD on discovery endpoints. The proxy returns only headers (no body) to the client.

### Backward Compatibility

OAuth support is **opt-in** at the upstream level. If the upstream server doesn't support OAuth:
- Discovery endpoints return 404 (forwarded from upstream)
- The proxy continues to work normally for non-OAuth flows
- No configuration changes needed

### Testing OAuth

Use the provided test script to verify OAuth discovery:
...
# Verify URLs are rewritten (should NOT contain upstream domain)
...
**Expected behavior:**
- HEAD requests return `200 OK` with headers (not `405 Method Not Allowed`)
- Discovery documents contain proxy URLs, not upstream URLs
- If upstream is unreachable, proxy returns `502 Bad Gateway`
- If upstream doesn't support OAuth, proxy returns `404 Not Found`
