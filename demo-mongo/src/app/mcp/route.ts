...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Forward Accept - pass through client preference, default to JSON
    ...
    // Passthrough non-widget requests (allow streaming)
    if (!isToolsCallRequest(body)) {
      const responseHeaders = new Headers(response.headers);
      const auth = responseHeaders.get("WWW-Authenticate");
      if (auth) {
        const proxyBase = getBaseUrl(request);
        const upstreamOrigin = new URL(MCP_SERVER_URL).origin;
        responseHeaders.set(
          "WWW-Authenticate",
          auth.replace(upstreamOrigin, proxyBase)
        );
      }
    ...
    // For widget requests, we must parse JSON to inject metadata
    ...
    try {
      ...
      console.error(`[MCP Proxy] Invalid JSON from upstream: ${text}`);
      throw e;
    }

    // Intercept tool call responses and inject widget metadata
    if (isToolsCallResponse(data)) {
    ...
    if (wwwAuth) {
      ...
      responseHeaders.set(
        "WWW-Authenticate",
        wwwAuth.replace(upstreamOrigin, proxyBase)
      );
    }
